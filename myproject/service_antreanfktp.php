...
function nextQueueNumber($kodePoli, $tanggal) {
    // Nanti bisa diubah sesuai logika antrean real
...
while (true) {
    ...
    if ($now->format('H:i') >= '07:00' && $now->format('H:i') <= '20:00') {
        ...
        rp.no_rawat, 
        rp.no_reg AS nomor, 
        RIGHT(no_reg,3) AS angka, 
        ps.nm_pasien, 
        ps.no_tlp AS nohp, 
        ps.no_rkm_medis,
               ps.no_peserta, ps.no_ktp, rp.kd_poli, pk.nm_poli, 
               mdk.kd_dokter_pcare, 
               d.nm_dokter,
            ...
            foreach ($cari_pasien as $data) {
                try {
                    ...
                    if (curl_errno($ch)) {
                        ...
                        writeLog("Pasien dikirim: " . $data['nm_pasien'] . " => Response: " . $response);

                        // ---------- Catat ke tabel ----------
                        $insert = "
                            INSERT INTO antrean_terkirim_bpjs (no_rawat, tgl_kirim, response)
                            VALUES (?, NOW(), ?)
                            ON DUPLICATE KEY UPDATE response = VALUES(response), tgl_kirim = NOW()
                        ";
                        queryPrepared($insert, [$data['no_rawat'], $response]);
                    }
                    curl_close($ch);
                } catch (Exception $e) {
